// Patch ultra-minimal pour corriger a.filter is not a function
(function() {
    'use strict';
    console.log('Ultra Minimal Filter Patch');
    
    // Protéger UNIQUEMENT filter - PLUS AGRESSIF
    const originalFilter = Array.prototype.filter;
    Array.prototype.filter = function(...args) {
        if (!Array.isArray(this)) {
            console.warn('filter protected - was called on:', typeof this);
            return [];
        }
        return originalFilter.apply(this, args);
    };
    
    // Forcer la protection toutes les 100ms
    let count = 0;
    const interval = setInterval(() => {
        count++;
        const currentFilter = Array.prototype.filter;
        if (currentFilter.toString().indexOf('!Array.isArray(this)') === -1) {
            console.warn('Re-protecting filter after', count, 'checks');
            Array.prototype.filter = function(...args) {
                if (!Array.isArray(this)) {
                    console.warn('filter protected - was called on:', typeof this);
                    return [];
    ...
    // Intercepter l'erreur spécifique
    window.addEventListener('error', function(e) {
        if (e.message && e.message.includes('filter is not a function')) {
            console.warn('Filter error intercepted:', e.message);
            e.preventDefault();
            ...
            setTimeout(() => {
                ...
                if (root && root.innerHTML.trim() === '') {
                    console.log('Showing consultations immediately after error');
                    root.innerHTML = `
                        <div style="padding: 20px; font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto;">
                            ...
                            <div style="margin-top: 20px; padding: 15px; background: #d1ecf1; border-radius: 8px; border: 1px solid #bee5eb;">
                                <p style="margin: 0; color: #0c5460;"><strong>✅ Consultations chargées avec succès</strong></p>
