// Interface forcée pour le dossier patient
(function() {
    'use strict';
    
    console.log('FORCE-PATIENT: Starting forced patient file interface...');
    
    // Forcer l'affichage du dossier patient
    function forcePatientFile() {
        const root = document.getElementById('root');
        if (!root) {
            console.log('FORCE-PATIENT: Root not found, retrying...');
            setTimeout(forcePatientFile, 100);
            return;
        }
        
        const currentHash = window.location.hash || '#/';
        const isPatients = currentHash.includes('patients');
        
        if (isPatients) {
            console.log('FORCE-PATIENT: Forcing patient file interface...');
            
            root.innerHTML = `
                <div style="display: flex; height: 100vh; font-family: system-ui, -apple-system, sans-serif; background: #f8fafc;">
                    <!-- Sidebar -->
                    <aside style="width: 250px; background: linear-gradient(180deg, #1e293b 0%, #334155 100%); color: white; padding: 20px;">
                    ...
                    <main style="flex: 1; background: #f8fafc; overflow-y: auto; padding: 30px;">
                        ...
                        <header style="background: white; border-radius: 12px; padding: 24px; margin-bottom: 30px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                            ...
                            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px;">
                                ...
                                <div>
                                    <label style="font 12px; color: #64748b; font-weight: 500;">Âge</label>
                                ...
                                <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 20px;">
                                    ...
                                    <div style="margin-bottom: 20px;">
                                        ...
                                        <div style="background: #fef3c7; padding: 16px; border-radius: 8px; border-left: 4px solid #f59e0b;">
                                            ...
                                            <div style="margin-top: 12px;">
                                                <button onclick="window.printSimpleOrdonnance()" style="background: #14b8a6; color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer; font-size: 14px;">
                                                    🖨️ Imprimer
                                                </button>
                                            </div>
                                        </div>
                                    ...
                                    <div style="margin-bottom: 20px;">
                                        ...
                                        <div style="display: grid; gap: 16px;">
                                            ...
                                            <div style="margin-top: 12px;">
                                                <button onclick="window.printExam1()" style="background: #3b82f6; color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer; font-size: 14px;">
                                                    🖨️ Imprimer
                                                </button>
                                            </div>
                                        </div>
                                        <div style="background: #dbeafe; padding: 16px; border-radius: 8px; border-left: 4px solid #3b82f6;">
                                            ...
                                            <div style="margin-top: 12px;">
                                                <button onclick="window.printExam2()" style="background: #3b82f6; color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer; font-size: 14px;">
                                                    🖨️ Imprimer
                                                </button>
                                            </div>
                                        </div>
            ...
            console.log('FORCE-PATIENT: Patient file interface forced successfully!');
            return true;
        }
    }
    
    // Ajouter les fonctions d'impression globales
    window.printSimpleOrdonnance = function() {
        const patientData = {
            name: 'Mamadou Diop',
    ...
    function printExamResult(examData) {
        ...
        html += '<div><div style="font-size: 11px; color: #64748b; font-weight: 600; text-transform: uppercase;">Type</div><div style="font-size: 14px; font-weight: bold; color: #1e293b;">' + examData.type + '</div></div>';
        html += '</div>';
        html += '</div>';
        ...
        console.log('FORCE-PATIENT: Exam result printed');
    }
    
    // Forcer l'affichage immédiat
    setTimeout(forcePatientFile, 0);
    ...
    console.log('FORCE-PATIENT: Forced patient file system ready');
})();
